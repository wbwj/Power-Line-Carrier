...
{   
    ...
    while (DHT11_DQ_IN_Indoor&&retry<100)//DHT11会拉低40~80us
	{
		retry++;
		delay_us(1);
	};	 
    ...
    while (!DHT11_DQ_IN_Indoor&&retry<100)//DHT11拉低后会再次拉高40~80us
	{
		retry++;
		delay_us(1);
	};
...
{   
    ...
    while (DHT11_DQ_IN_Outdoor&&retry<100)//DHT11会拉低40~80us
	{
		retry++;
		delay_us(1);
	};	 
    ...
    while (!DHT11_DQ_IN_Outdoor&&retry<100)//DHT11拉低后会再次拉高40~80us
	{
		retry++;
		delay_us(1);
	};
...
{
 	u8 retry=0;
	while(DHT11_DQ_IN_Indoor&&retry<100)//等待变为低电平
	{
		retry++;
		delay_us(1);
	}
	retry=0;
	while(!DHT11_DQ_IN_Indoor&&retry<100)//等待变高电平
	{
		retry++;
		delay_us(1);
	}
...
{
 	u8 retry=0;
	while(DHT11_DQ_IN_Outdoor&&retry<100)//等待变为低电平
	{
		retry++;
		delay_us(1);
	}
	retry=0;
	while(!DHT11_DQ_IN_Outdoor&&retry<100)//等待变高电平
	{
		retry++;
		delay_us(1);
	}
...
{	 
	...
	DHT11_Rst_Indoor();  //复位DHT11
	return DHT11_Check_Indoor();//等待DHT11的回应
} 
...
{	 
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;				 //PG11端口配置
 	...
 	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOG, &GPIO_InitStructure);				 //初始化IO口
 	GPIO_SetBits(GPIOG,GPIO_Pin_12);						 //PG11 输出高
			    
	DHT11_Rst_Outdoor();  //复位DHT11
	return DHT11_Check_Outdoor();//等待DHT11的回应
} 
